...
def main():
    ...
    optimizer = optim.AdamW([a_torch, b_torch], lr=1e-4, eps=1e-08, weight_decay=0.01)
    ...
    for epoch in range(30):      
        ...
        for i in range(vals_ns.shape[0]//batch_size):
            ...
            loss, g_a, g_b, g_c = gcp_grad(
                torch.tensor(coo_ns[i*batch_size : end], device = device), torch.tensor(vals_ns[i*batch_size : end], device = device), shape, a_torch, b_torch, l2, loss_function, loss_function_grad, device)
